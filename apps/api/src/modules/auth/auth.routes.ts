...
export async function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/auth/change-password', {
    ...
    if (!staff) return reply.status(404).send({ message: 'User not found' })

    const valid = await bcrypt.compare(currentPassword, staff.pwHash)
    ...
    await prisma.staff.update({
      where: { id: user.id },
      data:  { pwHash, mustChangePassword: false } as any
    })
  ...
  fastify.post('/auth/reset-password', async (request, reply) => {
    ...
    if (!staff) {
      return reply.status(400).send({ message: 'Invalid or expired reset link. Please request a new one.' })
    }
