...
export default function Dashboard() {
  ...
  // Pro bono calculations
  const proBonoClients   = clients.filter(c => c.isProBono)
  const proBonoIds       = new Set(proBonoClients.map(c => c.id))
  const proBonoSessions  = appointments.filter(a => {
    const cid = a.clientId ?? a.client?.id
    return cid && proBonoIds.has(cid)
  })
  const completedPB      = proBonoSessions.filter(a => a.status === "COMPLETED")
  ...
  return (
    <Layout title="Dashboard">
      ...
      {/* Pro Bono summary bar — only shown if there are pro bono clients */}
      {proBonoClients.length > 0 && (
        <div style={{ background:"linear-gradient(135deg,#fef3c7,#fde68a)", border:"1px solid #fbbf24", borderRadius:14, padding:"14px 20px", marginBottom:14, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:20 }}>
            ...
            <a href="/reports" style={{ alignSelf:"center", padding:"6px 14px", borderRadius:8, border:"none", background:"#d97706", color:"white", fontSize:12.5, fontWeight:500, textDecoration:"none", cursor:"pointer" }}>
              Full Report →
            </a>
          </div>
        </div>
          ...
          <div style={{ padding:"14px 18px", borderBottom:"1px solid #d6e8e0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              <div style={{ fontSize:13.5, fontWeight:600, color:"#1a2724" }}>
                Today's Appointments — {today.toLocaleDateString("en-KE",{ weekday:"short", day:"numeric", month:"short" })}
              </div>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, fontWeight:500, color:"#1a2724" }}>
                      ...
                      {(() => {
                        ...
                        return isPB ? <span style={{ fontSize:10, marginLeft:6, padding:"1px 6px", borderRadius:20, background:"#fef3c7", color:"#d97706", fontWeight:600 }}>Pro Bono</span> : null
                      })()}
                    </div>
            ...
            <div style={{ padding:"14px 18px" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3, textAlign:"center" }}>
                {Array.from({ length:35 }, (_, i) => {
                  ...
                  const hasAppt    = valid && appointments.some(a =>
          ...
          <div style={{ padding:"0 18px" }}>
            {[
              ...clients.slice(0,2).map(c => ({
                icon:"👤",
                text:`${c.fullName} registered as a client${c.isProBono?" (Pro Bono)":""}`,
                time: new Date(c.createdAt).toLocaleDateString("en-KE",{ day:"numeric", month:"short" }),
                bg: c.isProBono ? "#fef3c7" : "#e6f4ef"
              })),
              ...appointments.slice(0,3).map(a => ({
                icon:"📅",
                text:`${a.client?.fullName ?? "Client"} — ${a.therapyType} session booked`,
        ...
        <div style={{ background:"white", border:"1px solid #d6e8e0", borderRadius:16, overflow:"hidden" }}>
          ...
          <div style={{ padding:"12px 18px", display:"flex", flexDirection:"column", gap:8 }}>
            ...
            {proBonoClients.length > 0 && (
              <div style={{ display:"flex", gap:10, padding:"10px 12px", borderRadius:10, background:"#fef3c7" }}>
                <span>🤝</span>
                <div style={{ fontSize:12.5, color:"#92400e" }}>{proBonoClients.length} pro bono {proBonoClients.length===1?"client":"clients"} — KSh {valueWaived.toLocaleString()} waived</div>
              </div>
            )}
