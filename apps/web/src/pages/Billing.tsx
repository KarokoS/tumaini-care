...
export default function Billing() {
  ...
  const [editInvItems, setEditInvItems]                 = useState<{ description:string; quantity:string; unitPrice:string }[]>([])
  ...
  function getTotal() {
    return items.reduce((s,item) => s + (parseInt(item.quantity)||0) * (parseFloat(item.unitPrice)||0), 0)
  }
  ...
  function openMpesa(inv: any) {
    setMpesaInvoice(inv); setMpesaPhone(""); setMpesaStatus("idle"); setMpesaMessage(""); setShowMpesa(true)
  }

  async function sendStkPush(e: React.FormEvent) {
    e.preventDefault(); setMpesaLoading(true); setMpesaStatus("idle"); setMpesaMessage("")
    try {
      ...
      setMpesaStatus("failed"); setMpesaMessage(err.response?.data?.message ?? "Failed to send STK push")
  ...
  function pollStatus(cid: string) {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        if (res.data.paid) {
          clearInterval(interval); setMpesaStatus("success"); setMpesaMessage("Payment confirmed! ✓"); loadData()
        } else if (attempts >= 6) {
          clearInterval(interval); setMpesaStatus("failed"); setMpesaMessage("Payment not confirmed after 30 seconds.")
        }
  ...
  async function deleteInvoice(inv: any) {
    ...
    try {
      ...
      setConfirmDeleteInvoice(null); loadData()
    } catch (err:any) { alert(err.response?.data?.message ?? "Failed to delete invoice") }
  ...
  return (
    <Layout title="Billing & Payments" action={
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:20 }}>
        {[
          { label:"Total Collected",   value:"KSh "+totalRevenue.toLocaleString(),     color:"#1a8c6e", sub:invoices.filter(i=>i.status==="PAID"&&parseFloat(i.amountKes)>0).length+" paid" },
          { label:"Outstanding",       value:"KSh "+totalOutstanding.toLocaleString(), color:"#d63f5c", sub:invoices.filter(i=>i.status!=="PAID"&&parseFloat(i.amountKes)>0).length+" unpaid" },
          { label:"Total Invoiced",    value:"KSh "+invoices.filter(i=>parseFloat(i.amountKes)>0).reduce((s,i)=>s+parseFloat(i.amountKes),0).toLocaleString(), color:"#2563a8", sub:invoices.length+" total" },
          { label:"Pro Bono Sessions", value:proBonoCount+" sessions", color:"#d97706", sub:"no charge" },
        ].map((s,i) => (
            ...
            <form onSubmit={saveInvoice}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
                <div style={{ gridColumn:"span 2" }}>
                  ...
                  <select required value={clientId} onChange={e=>handleClientChange(e.target.value)}
                    ...
                    {clients.map((c,i) => <option key={i} value={c.id}>{c.fullName}{c.isProBono?" 🤝 Pro Bono":""}</option>)}
                  </select>
                  {selectedClient?.isProBono && (
                    <div style={{ marginTop:8, padding:"8px 12px", borderRadius:8, background:"#fef3c7", border:"1px solid #fbbf24", fontSize:12.5, color:"#92400e" }}>
                      🤝 Pro bono client — invoice will be KSh 0.
                    </div>
                  )}
                </div>
                <div>
                ...
                <div style={{ display:"flex", alignItems:"flex-end", paddingBottom:4 }}>
                  <label style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:"#1a2724", cursor:"pointer" }}>
                    <input type="checkbox" checked={isProBonoInvoice} onChange={e=>{
                      setIsProBonoInvoice(e.target.checked)
                      if (e.target.checked) setItems([{ description:"Pro Bono Session — No charge", quantity:"1", unitPrice:"0" }])
                      else setItems([{ description:"", quantity:"1", unitPrice:"" }])
                    }}/>
              ...
              <div style={{ marginBottom:16 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  ...
                  {!isProBonoInvoice && <button type="button" onClick={addItem} style={{ fontSize:12, color:"#1a8c6e", border:"none", background:"none", cursor:"pointer", fontWeight:500 }}>+ Add item</button>}
                </div>
                {items.map((item,i) => (
                  <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 80px 100px 32px", gap:8, marginBottom:8 }}>
                    ...
                    <input placeholder="KSh" type="number" value={item.unitPrice} onChange={e=>updateItem(i,"unitPrice",e.target.value)} required readOnly={isProBonoInvoice}
                      style={{ padding:"8px 12px", borderRadius:8, border:"1px solid #d6e8e0", fontSize:13, background:isProBonoInvoice?"#f8faf9":"white" }}/>
                    {!isProBonoInvoice && <button type="button" onClick={() => removeItem(i)} style={{ border:"none", background:"#fde8ed", color:"#d63f5c", borderRadius:8, cursor:"pointer" }}>×</button>}
                  </div>
                ))}
              ...
              <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
                ...
                <button type="submit" disabled={saving} style={{ padding:"9px 16px", borderRadius:8, border:"none", background:isProBonoInvoice?"#d97706":"#1a8c6e", color:"white", fontSize:13, fontWeight:500, cursor:"pointer", opacity:saving?0.7:1 }}>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }}>
          <div style={{ background:"white", borderRadius:16, padding:28, width:440 }}>
            ...
            <div style={{ background:"#f8faf9", borderRadius:10, padding:"12px 16px", marginBottom:20 }}>
              {[["Invoice",mpesaInvoice.number],["Client",mpesaInvoice.client?.fullName],["Amount","KSh "+parseFloat(mpesaInvoice.amountKes).toLocaleString()]].map(([label,value],i) => (
            ...
            {mpesaMessage && (
              <div style={{ padding:"12px 16px", borderRadius:10, marginBottom:16, fontSize:13,
                ...
                {mpesaStatus==="pending"&&"⏳ "}{mpesaStatus==="success"&&"✓ "}{mpesaStatus==="failed"&&"✗ "}{mpesaMessage}
              </div>
            )}
            ...
            <div style={{ background:"#e6f4ef", borderRadius:10, padding:"14px 16px", marginBottom:16 }}>
              <div style={{ fontSize:11, fontWeight:600, color:"#1a8c6e", textTransform:"uppercase", marginBottom:10 }}>M-Pesa Paybill Instructions</div>
              {[["Business Number","880100"],["Account Number","411511"],["Amount","KSh "+parseFloat(mpesaInvoice.amountKes).toLocaleString()]].map(([label,value],i) => (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:12.5, marginBottom:6 }}>
                  <span style={{ color:"#4a6359" }}>{label}</span>
                  <span style={{ fontWeight:700, color:"#1a2724" }}>{value}</span>
                </div>
              ))}
            </div>
            <div style={{ display:"flex", gap:8 }}>
            ...
            <form onSubmit={saveEditInvoice}>
              ...
              <div style={{ marginBottom:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                  <label style={{ fontSize:12, color:"#4a6359", fontWeight:500 }}>Line items</label>
                  <button type="button"
                    onClick={() => setEditInvItems(p=>[...p,{ description:"", quantity:"1", unitPrice:"" }])}
                    style={{ fontSize:12, color:"#1a8c6e", border:"none", background:"none", cursor:"pointer", fontWeight:500 }}>
                    + Add item
                  </button>
                </div>
                {editInvItems.map((item,i) => (
                  <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 80px 100px 32px", gap:8, marginBottom:8 }}>
                    <input value={item.description}
                      onChange={e=>setEditInvItems(p=>p.map((it,idx)=>idx===i?{...it,description:e.target.value}:it))}
                      placeholder="Description" style={{ padding:"8px 12px", borderRadius:8, border:"1px solid #d6e8e0", fontSize:13 }}/>
                    <input type="number" value={item.quantity}
                      onChange={e=>setEditInvItems(p=>p.map((it,idx)=>idx===i?{...it,quantity:e.target.value}:it))}
                      placeholder="Qty" style={{ padding:"8px 12px", borderRadius:8, border:"1px solid #d6e8e0", fontSize:13 }}/>
                    <input type="number" value={item.unitPrice}
                      onChange={e=>setEditInvItems(p=>p.map((it,idx)=>idx===i?{...it,unitPrice:e.target.value}:it))}
                ...
                <div style={{ textAlign:"right", fontSize:14, fontWeight:600, color:"#1a2724", marginTop:8 }}>
                  Total: KSh {editInvItems.reduce((s,item) => s+(parseInt(item.quantity)||0)*(parseFloat(item.unitPrice)||0), 0).toLocaleString()}
                </div>
              </div>
              <div style={{ marginBottom:16 }}>
                <label style={{ fontSize:12, color:"#4a6359", display:"block", marginBottom:4 }}>Notes</label>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
          <div style={{ background:"white", borderRadius:16, padding:28, width:420 }}>
            ...
            <div style={{ fontSize:13, color:"#4a6359", marginBottom:20, lineHeight:1.6 }}>
              Permanently delete invoice <strong>{confirmDeleteInvoice.number}</strong> for <strong>{confirmDeleteInvoice.client?.fullName}</strong>. This cannot be undone.
            </div>
            <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
              <button onClick={() => setConfirmDeleteInvoice(null)} style={{ padding:"9px 16px", borderRadius:8, border:"1px solid #d6e8e0", background:"white", fontSize:13, cursor:"pointer", color:"#4a6359" }}>Cancel</button>
              <button onClick={() => deleteInvoice(confirmDeleteInvoice)} disabled={deletingInvoice} style={{ padding:"9px 16px", borderRadius:8, border:"none", background:"#d63f5c", color:"white", fontSize:13, fontWeight:500, cursor:"pointer", opacity:deletingInvoice?0.7:1 }}>
