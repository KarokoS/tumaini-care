...
export default function Billing() {
  ...
  // Auto-detect pro bono when client is selected
  ...
  const proBonoCount     = invoices.filter(i => parseFloat(i.amountKes) === 0).length

  const filteredInvoices = invoices.filter(inv => {
    if (filterStatus === "PAID")    return inv.status === "PAID"
    if (filterStatus === "UNPAID")  return inv.status !== "PAID"
    if (filterStatus === "PROBONO") return parseFloat(inv.amountKes) === 0
    return true
  })
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tbody>
            {loading ? (
              ...
              const isProBono = parseFloat(inv.amountKes) === 0
              return (
                <tr key={i} style={{ borderBottom: "1px solid #f0f4f2", background: isProBono ? "#fffbeb" : "white" }}>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <span style={{ fontSize: 11.5, padding: "2px 8px", borderRadius: 20, background: "#f0f4f2", color: "#4a6359", fontWeight: 500 }}>
                      {inv.mpesaRef ? "M-Pesa" : inv.status === "PAID" ? "Manual" : isProBono ? "Pro Bono" : "—"}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 20, fontWeight: 500, background: inv.status === "PAID" ? "#e6f4ef" : inv.status === "OVERDUE" ? "#fde8ed" : isProBono ? "#fef3c7" : "#fef3c7", color: inv.status === "PAID" ? "#1a8c6e" : inv.status === "OVERDUE" ? "#d63f5c" : isProBono ? "#d97706" : "#d97706" }}>
            ...
            <form onSubmit={saveInvoice}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                <div style={{ gridColumn: "span 2" }}>
                  ...
                  <select required value={clientId} onChange={e => handleClientChange(e.target.value)} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13, boxSizing: "border-box" }}>
                    <option value="">Select client...</option>
                    {clients.map((c, i) => (
                      <option key={i} value={c.id}>
                        {c.fullName}{c.isProBono ? " 🤝 Pro Bono" : ""}
                      </option>
                    ))}
                  </select>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <label style={{ fontSize: 12, color: "#4a6359", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", marginTop: 20 }}>
                    <input
                      type="checkbox"
                      checked={isProBonoInvoice}
                      onChange={e => {
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {items.map((item, i) => (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 80px 100px 32px", gap: 8, marginBottom: 8 }}>
                    <input placeholder="Description" value={item.description} onChange={e => updateItem(i, "description", e.target.value)} required readOnly={isProBonoInvoice} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13, background: isProBonoInvoice ? "#f8faf9" : "white" }} />
                    <input placeholder="Qty" type="number" value={item.quantity} onChange={e => updateItem(i, "quantity", e.target.value)} readOnly={isProBonoInvoice} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13, background: isProBonoInvoice ? "#f8faf9" : "white" }} />
                    <input placeholder="KSh" type="number" value={item.unitPrice} onChange={e => updateItem(i, "unitPrice", e.target.value)} required readOnly={isProBonoInvoice} style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13, background: isProBonoInvoice ? "#f8faf9" : "white" }} />
                    {!isProBonoInvoice && (
      ...
      {showMpesa && mpesaInvoice && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 440 }}>
            ...
            {mpesaStatus !== "success" && (
              <form onSubmit={sendStkPush}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                  <input required value={mpesaPhone} onChange={e => setMpesaPhone(e.target.value)} placeholder="07XX XXX XXX" style={{ flex: 1, padding: "10px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13 }} />
                  <button type="submit" disabled={mpesaLoading || mpesaStatus === "pending"} style={{ padding: "10px 16px", borderRadius: 8, border: "none", background: "#1a8c6e", color: "white", fontSize: 13, fontWeight: 500, cursor: "pointer", opacity: mpesaLoading ? 0.7 : 1, whiteSpace: "nowrap" }}>
            ...
            {mpesaMessage && (
              <div style={{ padding: "12px 16px", borderRadius: 10, marginBottom: 16, fontSize: 13, background: mpesaStatus === "success" ? "#e6f4ef" : mpesaStatus === "failed" ? "#fde8ed" : "#fef3c7", color: mpesaStatus === "success" ? "#1a8c6e" : mpesaStatus === "failed" ? "#d63f5c" : "#d97706", border: `1px solid ${mpesaStatus === "success" ? "#b6ddd1" : mpesaStatus === "failed" ? "#f5b8c4" : "#fde68a"}` }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {mpesaStatus !== "success" && (
                <button onClick={() => markPaid(mpesaInvoice.id)} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "1px solid #d6e8e0", background: "white", fontSize: 12.5, cursor: "pointer", color: "#4a6359" }}>
                  Mark as paid manually
                </button>
              )}
              <button onClick={() => setShowMpesa(false)} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "none", background: mpesaStatus === "success" ? "#1a8c6e" : "#f0f4f2", color: mpesaStatus === "success" ? "white" : "#4a6359", fontSize: 12.5, fontWeight: 500, cursor: "pointer" }}>
