      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "white", border: "1px solid #d6e8e0", borderRadius: 16, padding: "20px 24px" }}>
          ...
          {[
            ...
            { label: "Registered",      value: client.registrationDate ? new Date(client.registrationDate).toLocaleDateString("en-KE") : new Date(client.createdAt).toLocaleDateString("en-KE") },
          ].map((row, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #f0f4f2", fontSize: 13 }}>
              ...
              <span style={{ color: "#1a2724", fontWeight: 400, textAlign: "right", maxWidth: "60%" }}>{row.value}</span>
            </div>
          ))}
        </div>

        {/* Guardian + sessions summary */}
          ...
          <div style={{ background: "white", borderRadius: 16, padding: 28, width: 560, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <form onSubmit={saveEdit}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                {[
                  { label: "Full Name *", key: "fullName", required: true, type: "text" },
                  { label: "Date of Birth *", key: "dob", required: true, type: "date" },
                  { label: "Diagnosis", key: "diagnosis", required: false, type: "text" },
                  { label: "Co-occurring conditions", key: "coOccurring", required: false, type: "text" },
                  { label: "Allergies / alerts", key: "allergies", required: false, type: "text" },
                  { label: "School name", key: "schoolName", required: false, type: "text" },
                  { label: "Referral source", key: "referralSrc", required: false, type: "text" },
                ].map((f, i) => (
                  <div key={i} style={f.key === "fullName" || f.key === "allergies" ? { gridColumn: "span 2" } : {}}>
                    <label style={{ fontSize: 12, color: "#4a6359", display: "block", marginBottom: 4 }}>{f.label}</label>
                    <input
                      type={f.type}
                      required={f.required}
                      value={form[f.key] ?? ""}
                ...
                <div>
                  ...
                  <select value={form.gender ?? ""} onChange={e => setForm((p: any) => ({ ...p, gender: e.target.value }))} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13, boxSizing: "border-box" }}>
                    <option>Male</option><option>Female</option><option>Other</option>
                  </select>
                </div>
                <div>
                  <label style={{ fontSize: 12, color: "#4a6359", display: "block", marginBottom: 4 }}>Status</label>
                  <select value={form.status ?? "ACTIVE"} onChange={e => setForm((p: any) => ({ ...p, status: e.target.value }))} style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #d6e8e0", fontSize: 13, boxSizing: "border-box" }}>
