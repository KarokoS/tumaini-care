...
export default function Reports() {
  ...
  // ── Derived date-range Date objects (used by annual report + new-clients table) ──
  // FIX: previously the "Newly Registered Clients" section referenced
  // `reportFromDate` / `reportToDate`, which were never defined anywhere,
  // causing a ReferenceError that crashed the whole page on render.
  const reportFromDate = new Date(reportFrom)
  const reportToDate = new Date(reportTo)
  ...
  // ── New clients registered within the selected report window ──
  // FIX: this used to live nested inside the Pro Bono block's IIFE, and that
  // block does `if (proBonoClients.length === 0) return null` — which meant
  // this whole table silently vanished whenever there were no pro bono
  // clients. Moved out so it always renders independently.
  const newClientsList = clients.filter((c: any) => {
    const d = new Date(c.createdAt)
    return d >= reportFromDate && d <= reportToDate
  })
    ...
    <Layout title="Reports & Analytics">
      {loading ? (
        ...
        <div style={{ background:"white", border:"1px solid #d6e8e0", borderRadius:14, padding:"18px 22px", marginBottom:20 }}>
          ...
          {/* FIX: moved out of the Pro Bono block so it renders regardless of
              whether there are any pro bono clients, and now uses the
              properly-defined reportFromDate / reportToDate. */}
          <div style={{ background:"white", border:"1px solid #d6e8e0", borderRadius:14, overflow:"hidden", marginTop:14 }}>
            <div style={{ padding:"14px 18px", borderBottom:"1px solid #d6e8e0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
